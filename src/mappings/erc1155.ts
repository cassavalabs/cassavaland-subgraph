...
export function handleTransferSingle(event: TransferSingle): void {
  ...
  registerTransfer(contract, from, to, event.params.id, event.params.value);
}
...
export function handleTransferBatch(event: TransferBatch): void {
  ...
  if (ids.length == values.length) {
    for (let i = 0; i < ids.length; ++i) {
      registerTransfer(contract, from, to, ids[i], values[i]);
    }
  }
}
...
export function handleURI(event: URI): void {
  let contract = loadERC1155Contract(event.address);
  let token = loadERC1155Nft(contract, event.params.id);
  token.metadataURL = replaceURI(event.params.value, event.params.id);
  token.save();
}
